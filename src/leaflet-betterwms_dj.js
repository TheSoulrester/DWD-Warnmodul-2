...
L.TileLayer.BetterWMS = L.TileLayer.WMS.extend({
  ...
  onAdd: function (map) {
    ...
    if(!this._marker) this._marker = L.marker([49.8916044, 10.8868478]).addTo(this._map)  //Marker hinzufügen
  },
...
/*    $.ajax({
  ...
  showGetFeatureInfoJson: function (latlng, data) {  var newstyle=true  //false edit here
    ...
    data.features.forEach(function(item){ item=item.properties  //$.each(data.features, function (i, item) {
        ...
        + "<tr><td hidden>Ereignis&nbsp;:</td><td><b><a style='text-decoration:none' href='?" + item.EC_GROUP + "'>" + item.EVENT.replace("RMATION","") + "</a></b>"  //.EVENT
